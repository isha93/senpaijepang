...
public struct MockDocumentUploadCard: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            .buttonStyle(.plain)
            .confirmationDialog("Upload Document".localized(), isPresented: $showActionSheet, titleVisibility: .visible) {
                Button("Take Photo".localized()) { simulateUpload(file: "Photo_\(Int.random(in: 1000...9999)).jpg") }
                Button("Photo Library".localized()) { simulateUpload(file: "Image_\(Int.random(in: 1000...9999)).jpg") }
                Button("Choose File".localized()) { simulateUpload(file: "\(title.replacingOccurrences(of: " ", with: "_"))_2026.pdf") }
                if case .success = state {
                    Button("Remove File".localized(), role: .destructive) {
                        state = .empty
                    }
                }
                Button("Cancel".localized(), role: .cancel) { }
            } message: {
                Text(String(format: "Select a source to upload your %@".localized(), title.lowercased()))
            }
        }
    }
        ...
        ZStack {
            ...
            case .uploading(let progress):
                VStack(spacing: 12) {
                    ...
                    Text("Uploading... \(Int(progress * 100))%".localized())
                        .font(.subheadline.bold())
                        .foregroundStyle(AppTheme.accent)
                }
                .padding(.vertical, 24)
            ...
            case .success(let fileName):
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("Successfully uploaded".localized())
                            .font(.caption)
                            .foregroundStyle(AppTheme.textTertiary)
                    }
